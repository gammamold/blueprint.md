...
function TierBadge({ tier }) {
  const map={lurker:{l:"LURKER",c:"#444"},contributor:{l:"CONTRIBUTOR",c:"#E8845A"},donor:{l:"SUPPORTER",c:"#FFD700"},admin:{l:"ADMIN",c:"#c084fc"}};
  const t=map[tier]||map.lurker;
  return <span style={{fontSize:9,padding:"2px 7px",borderRadius:2,border:`1px solid ${t.c}44`,color:t.c,letterSpacing:1.5}}>{t.l}</span>;
}
  ...
  return (
    <div className="card" onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      ...
      <div>
        ...
        <div style={{fontSize:10,color:"#333"}}>by {bp.author}</div>
      </div>
      <div style={{fontSize:12,color:"#555",lineHeight:1.6}}>{bp.description}</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
        {(bp.tags||[]).map(t=><span key={t} style={{fontSize:10,color:"#2a2a2a"}}>#{t}</span>)}
      </div>
...
function BuildModal({ bp, user, onClose }) {
  ...
  const modes = [
    { id:"key", icon:"🔑", label:"Own API Key", sub:"Your key, your cost. Never stored.", avail:true },
    { id:"credits", icon:"⚡", label:"Contributor Credits", sub:user?`${user.builds_left} builds remaining`:"Sign in to use credits", avail:!!user&&(user.tier==="contributor"||user.tier==="donor"||user.tier==="admin") },
    { id:"payg", icon:"☕", label:"Pay as You Go", sub:"Coming soon — will charge at API cost, no markup.", avail:false },
  ];
    ...
    try {
      ...
      const res = await fetch("/api/build", {
        ...
        body: JSON.stringify({
          blueprintId: bp.id,
          llm: llm.id,
          accessMode: mode === "key" ? "own_key" : "credits",
          userApiKey: mode === "key" ? apiKey : undefined,
        }),
      });
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20,overflowY:"auto"}}>
      <div style={{background:"#0b0b0b",border:"1px solid #1e1e1e",borderRadius:5,width:"100%",maxWidth:result?720:480,overflow:"hidden",margin:"auto",transition:"max-width .3s"}}>
        <div style={{padding:"15px 20px",borderBottom:"1px solid #141414",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:9,color:"#E8845A",letterSpacing:2,marginBottom:3}}>BUILD BLUEPRINT</div>
        ...
        {result ? (
          ...
          <div style={{padding:20,display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{fontSize:9,color:"#4ADE80",letterSpacing:2}}>✓ BUILD COMPLETE</div>
          ...
          <div style={{padding:20,display:"flex",flexDirection:"column",gap:18}}>
            <div>
              <div style={{fontSize:9,color:"#333",letterSpacing:2,marginBottom:9}}>01 — SELECT MODEL</div>
            ...
            <div>
              <div style={{fontSize:9,color:"#333",letterSpacing:2,marginBottom:9}}>02 — ACCESS MODE</div>
                ...
                {modes.map(m=>(
                  <button key={m.id} onClick={()=>m.avail&&setMode(m.id)}
                    ...
                    <div>
                      ...
                      <div style={{fontSize:9,opacity:.4,marginTop:2}}>{m.avail?m.sub:"Upload blueprints to unlock contributor access"}</div>
                    </div>
            ...
            {mode==="key"&&(
              <div>
                ...
                <div style={{fontSize:9,color:"#1e1e1e",marginTop:5}}>🔒 Used only for this session. Never stored or logged.</div>
              </div>
            )}

            {buildError&&(
              <div style={{fontSize:10,color:"#F87171",padding:"8px 12px",border:"1px solid #F8717133",borderRadius:3}}>{buildError}</div>
            )}
            ...
            <button onClick={handleBuild} disabled={!ready}
              ...
              {building ? <><span className="spinner"/><span>BUILDING...</span></> : !mode?"SELECT A MODE":`BUILD WITH ${llm.label.toUpperCase()}`}
            </button>
          </div>
        )}
      </div>
  ...
  return (
    <div style={{background:"#080808",border:`1px solid ${result.overall?"#4ADE8022":"#F8717122"}`,borderRadius:4,overflow:"hidden"}}>
      <div style={{padding:"10px 14px",borderBottom:"1px solid #0f0f0f",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:9,letterSpacing:2,color:"#333"}}>VALIDATION REPORT</span>
        <span style={{fontSize:10,color:result.overall?"#4ADE80":"#F87171",letterSpacing:1}}>
          {result.overall ? "✓ PASSED" : "✗ FAILED"}
        </span>
      </div>
      <div style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:12}}>
        {[{label:"SECURITY",data:result.security},{label:"QUALITY",data:result.quality}].map(({label,data})=>(
          <div key={label}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
              <span style={{fontSize:9,color:"#444",letterSpacing:2}}>{label}</span>
              <span style={{fontSize:9,color:data.pass?"#4ADE80":"#F87171"}}>{data.pass?"PASS":"FAIL"}</span>
            </div>
            <ScoreBar score={data.score} pass={data.pass}/>
            {data.issues?.length>0&&(
              <div style={{marginTop:6,display:"flex",flexDirection:"column",gap:3}}>
                {data.issues.map((iss,i)=>(
                  <div key={i} style={{fontSize:10,color:"#F87171",paddingLeft:10,borderLeft:"1px solid #F8717133"}}>⚠ {iss}</div>
                ))}
              </div>
            )}
            {data.suggestions?.length>0&&(
              <div style={{marginTop:4,display:"flex",flexDirection:"column",gap:3}}>
                {data.suggestions.map((s,i)=>(
                  <div key={i} style={{fontSize:10,color:"#888",paddingLeft:10,borderLeft:"1px solid #333"}}>→ {s}</div>
                ))}
              </div>
            )}
          </div>
...
function UploadModal({ onClose, onSubmit }) {
  ...
  const runScan = async () => {
    ...
    try {
      setValidation(await validateBlueprint(form.content));
    } catch(e) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await onSubmit({ ...form, completeness: validation?.quality?.score });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20,overflowY:"auto"}}>
      <div style={{background:"#0b0b0b",border:"1px solid #1e1e1e",borderRadius:5,width:"100%",maxWidth:480,overflow:"hidden",margin:"auto"}}>
        <div style={{padding:"15px 20px",borderBottom:"1px solid #141414",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:9,color:"#E8845A",letterSpacing:2,marginBottom:2}}>SHARE A BLUEPRINT</div>
            <div style={{fontSize:10,color:"#333"}}>Upload earns +1 karma & contributor build credits</div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",color:"#333",cursor:"pointer",fontSize:16}}>✕</button>
        </div>
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:10}}>
          <input placeholder="Blueprint title" {...inp("title")}/>
          <select {...inp("category")} style={{...inp("category").style,background:"#0a0a0a"}}>
            {CATEGORIES.filter(c=>c!=="All").map(c=><option key={c}>{c}</option>)}
          </select>
          <textarea placeholder="What does this blueprint build? Be specific." {...inp("description",3)}/>
          <input placeholder="Tags: JUCE, bass, C++ ..." {...inp("tags")}/>
          ...
          <div onClick={()=>document.getElementById("mdfile").click()}
            ...
            {form.file
              ?<div><div style={{color:"#E8845A",fontSize:12}}>📄 {form.file.name}</div><div style={{color:"#333",fontSize:10,marginTop:3}}>click to change</div></div>
              :<div><div style={{color:"#2a2a2a",fontSize:12,marginBottom:3}}>drop .md file or click to browse</div><div style={{color:"#1a1a1a",fontSize:10}}>only .md accepted</div></div>}
          ...
          {form.file && validation === null && (
            <button onClick={runScan}
              style={{padding:"10px",border:"1px solid #E8845A44",borderRadius:3,background:"#E8845A0a",color:"#E8845A",cursor:"pointer",fontFamily:"'Share Tech Mono',monospace",fontSize:11,letterSpacing:1}}>
              🔍 SCAN BLUEPRINT
            </button>
          )}
          ...
          {validation === "scanning" && (
            <div style={{background:"#080808",border:"1px solid #141414",borderRadius:4,padding:"16px",textAlign:"center"}}>
              <div style={{fontSize:10,color:"#E8845A",letterSpacing:2,marginBottom:6}}>SCANNING</div>
              <div style={{display:"flex",justifyContent:"center",gap:4}}>
                {[0,1,2,3,4].map(i=><div key={i} style={{width:4,height:4,borderRadius:"50%",background:"#E8845A",animation:`pulse 1.2s ${i*0.2}s ease-in-out infinite`}}/>)}
              </div>
              <div style={{fontSize:10,color:"#333",marginTop:8}}>Checking for security issues and completeness...</div>
            </div>
          )}
          ...
          {validation && validation !== "scanning" && (
            <>
              ...
              {!validation.overall && (
                <button onClick={runScan}
                  style={{padding:"8px",border:"1px solid #333",borderRadius:3,background:"transparent",color:"#555",cursor:"pointer",fontFamily:"'Share Tech Mono',monospace",fontSize:10,letterSpacing:1}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:4}}>
            <button onClick={onClose} style={{flex:1,padding:"10px",border:"1px solid #141414",borderRadius:3,background:"transparent",color:"#333",cursor:"pointer",fontFamily:"'Share Tech Mono',monospace",fontSize:11}}>Cancel</button>
            <button onClick={handleSubmit}
              ...
              {submitting?"SUBMITTING...":!form.file?"UPLOAD FILE FIRST":!validation||validation==="scanning"?"SCAN REQUIRED":!validation.overall?"FIX ISSUES FIRST":"PUBLISH"}
            </button>
          </div>
        </div>
      </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}}>
      <div style={{background:"#0b0b0b",border:"1px solid #1e1e1e",borderRadius:5,width:"100%",maxWidth:360,padding:28}}>
        ...
        <div style={{fontSize:9,color:"#333",letterSpacing:1,marginBottom:20}}>
          {mode==="signin"?"Welcome back.":"Upload blueprints, earn karma, build things."}
        </div>

        {mode==="signup"&&<input {...inp(username,setUsername,"Username")}/>}
        <input {...inp(email,setEmail,"Email","email")}/>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}}>
      <div style={{background:"#0b0b0b",border:"1px solid #1e1e1e",borderRadius:5,width:"100%",maxWidth:360,padding:28,textAlign:"center"}}>
        ...
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,letterSpacing:3,color:"#eee",marginBottom:8}}>BUY ME A COFFEE</div>
        <div style={{fontSize:11,color:"#444",lineHeight:1.8,marginBottom:20}}>
          This platform is free and runs at cost price.<br/>
          No VC money. No ads. Just the community.<br/>
          <span style={{color:"#333"}}>Donors get a supporter badge + extra build credits.</span>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
          {[["☕  One coffee","$3"],["☕☕  Two coffees","$6"],["🍺  A beer","$10"]].map(([label,price])=>(
        ...
        <button onClick={onClose} style={{background:"none",border:"none",color:"#222",cursor:"pointer",fontSize:10,fontFamily:"'Share Tech Mono',monospace"}}>no thanks, maybe later</button>
      </div>
    </div>
  );
}
...
export default function Marketplace() {
  ...
  // Derive display user from real profile
  const user = profile ? {
  ...
  // Load blueprints from Supabase on mount
  useEffect(() => {
    if (!supabase) {
      setBpLoading(false);
      return;
    ...
    bpHelpers.list({ limit: 100 })
      .then(data => {
        setBpList(data.map(bp => ({
          ...
          tags: bp.tags || [],
          downloads: bp.download_count || 0,
          builds: bp.build_count || 0,
  ...
  const handleUpload = async (form) => {
    ...
    try {
      await bpHelpers.create({
        ...
        tags: form.tags,
        tech_stack: [],
        completeness: form.completeness || 70,
        authorId: authUser.id,
      });
      setShowUpload(false);
      setUploadMsg("Blueprint submitted for review. It will appear in the marketplace once approved.");
  ...
  const handleBuildClick = (bp) => {
    if (!authUser && !bp._allowAnon) {
      setBuildTarget(bp);
    } else {
      setBuildTarget(bp);
    }
    ...
    <div style={{minHeight:"100vh",position:"relative"}}>
        ...
        {!authLoading && (
          user ? (
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <button onClick={signOut} className="btn-ghost" style={{padding:"5px 9px",border:"1px solid #1a1a1a",borderRadius:3,color:"#222",fontSize:10,letterSpacing:1}}>OUT</button>
            </div>
          ) : (
            <button onClick={()=>setShowAuth(true)} className="btn-ghost" style={{padding:"5px 14px",border:"1px solid #333",borderRadius:3,color:"#444",fontSize:10,letterSpacing:1}}>SIGN IN</button>
          )
        )}
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div style={{padding:"44px 24px 32px",borderBottom:"1px solid rgba(255,255,255,.04)",maxWidth:800}}>
          <div style={{fontSize:9,color:"#E8845A",letterSpacing:3,marginBottom:10}}>OPEN · FREE · COMMUNITY DRIVEN</div>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:44,lineHeight:1,color:"#eee",letterSpacing:3,marginBottom:14}}>
            SHARE A SPEC.<br/>BUILD ANYTHING.
          </div>
          <div style={{fontSize:12,color:"#444",lineHeight:1.9,maxWidth:480}}>
            Community <span style={{color:"#555"}}>.md blueprints</span> for VSTs, apps, UIs and tools.<br/>
            Build with Claude, Gemini or ChatGPT — use your own key, contributor credits, or pay a few cents at cost price. No markup ever.
          </div>
          <div style={{display:"flex",gap:18,marginTop:18}}>
            {[["📄",`${bpList.length} blueprints`],["⚡","builds tracked"],["👥","community contributors"]].map(([icon,label])=>(
              <div key={label} style={{fontSize:10,color:"#2a2a2a"}}>{icon} {label}</div>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:"16px 24px",borderBottom:"1px solid rgba(255,255,255,.04)",display:"flex",alignItems:"center",gap:0,flexWrap:"wrap"}}>
          {[["01","BROWSE","Find a blueprint for what you want to build"],["02","BUILD","Pick your LLM and access mode. Build instantly."],["03","CONTRIBUTE","Upload your own to earn build credits."]].map(([n,t,d],i,arr)=>(
            <div key={n} style={{display:"flex",alignItems:"center"}}>
              <div style={{padding:"10px 18px"}}>
                <div style={{fontSize:9,color:"#E8845A33",letterSpacing:1}}>{n}</div>
                <div style={{fontSize:11,color:"#555",letterSpacing:2}}>{t}</div>
                <div style={{fontSize:10,color:"#2a2a2a",marginTop:2,maxWidth:140}}>{d}</div>
              </div>
          ...
          <div style={{display:"flex",gap:5,flexShrink:0}}>
            {["🔒 HTTPS","🇪🇺 GDPR","🔑 Keys never stored"].map(t=>(
              <span key={t} style={{fontSize:9,color:"#1e1e1e",padding:"3px 8px",border:"1px solid #141414",borderRadius:2,letterSpacing:1,whiteSpace:"nowrap"}}>{t}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:"14px 24px",borderBottom:"1px solid rgba(255,255,255,.04)",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="search blueprints, tags, categories..."
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
            {CATEGORIES.map(c=>(
              <button key={c} onClick={()=>setCategory(c)} style={{padding:"6px 11px",borderRadius:3,border:`1px solid ${category===c?"#E8845A44":"rgba(255,255,255,.04)"}`,fontSize:10,cursor:"pointer",fontFamily:"'Share Tech Mono',monospace",letterSpacing:1,background:category===c?"#E8845A0a":"transparent",color:category===c?"#E8845A":"#333",transition:"all .15s"}}>{c}</button>
            ))}
          </div>
          <span style={{fontSize:9,color:"#1e1e1e",marginLeft:"auto"}}>{filtered.length} results</span>
        </div>
        ...
        <div style={{padding:"20px 24px",display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(270px, 1fr))",gap:10}}>
          {bpLoading ? (
            ...
            filtered.map(bp=><BlueprintCard key={bp.id} bp={bp} onBuild={handleBuildClick}/>)
          ) : (
            <div style={{gridColumn:"1/-1",textAlign:"center",padding:"60px 0",color:"#1e1e1e",fontSize:12}}>
              {!supabase ? (
                <>Add <code style={{background:"#111",padding:"2px 6px",borderRadius:2}}>NEXT_PUBLIC_SUPABASE_URL</code> and <code style={{background:"#111",padding:"2px 6px",borderRadius:2}}>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> to <code style={{background:"#111",padding:"2px 6px",borderRadius:2}}>.env.local</code>, then restart <code style={{background:"#111",padding:"2px 6px",borderRadius:2}}>npm run dev</code>.</>
              ) : (
                <>Nothing found. <span style={{color:"#E8845A",cursor:"pointer"}} onClick={()=>setShowUpload(true)}>Be the first to upload →</span></>
              )}
            </div>
          )}
        </div>
        ...
        <div style={{padding:"18px 24px",borderTop:"1px solid rgba(255,255,255,.04)",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
          ...
          <div style={{fontSize:9,color:"#1a1a1a",letterSpacing:1}}>FREE · OPEN · BUILT TO DEMOCRATIZE AI BUILDING</div>
          <div style={{display:"flex",gap:12}}>
            {["Privacy","Terms","GitHub"].map(l=><span key={l} style={{fontSize:9,color:"#1e1e1e",cursor:"pointer"}}>{l}</span>)}
          </div>
        </div>
      </div>
